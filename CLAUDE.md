...
Build a secure, self-hosted **multi-bot omnichannel customer-service platform**. The operator experience is intentionally simple: **Create → Connect → Teach → Go Live**. Channel security, Router9, dynamic skills, AI routing, workflows and credentials remain behind that product flow.
...
9. Runtime custom skills are **instructions-only** until sandboxing, approval, resource limits and audit controls exist. Never execute an imported skill script automatically.
10. Built-in runtime skill slugs cannot be overwritten; per-bot allowlists and tool-policy denies must not be bypassed by fallback code.
11. Conversation memory and traces must remain scoped and privacy-minimized; do not persist raw webhook bodies as memory/traces.
12. After code changes run `npm run check` and `npm test`.
13. Do not merge to `main` unless CI is green and security-sensitive connector/runtime changes have negative-path tests.
14. Prefer small, reversible and self-written implementation. If third-party code is incorporated, record source + license in `docs/SOURCES-LICENSES.md` first.
15. `nextlevelbuilder/goclaw` is CC BY-NC 4.0 and is a conceptual architecture reference only. Do not copy its source, bundled skill prose, prompts, regex sets or UI assets into this repository.
...
- Every bot owns its identity, intelligence mode, channels, knowledge, runtime skills, tool policy and scenario configuration.

## Architecture boundaries

- `src/connectors/`: webhook verification, normalization, outbound channel calls.
- `src/core/router9.js`: orchestration only; no channel-specific payload rules.
...
- `src/core/conversation-memory.js`: bounded process-local working history, not durable customer memory.
- `src/core/tool-policy.js`: capability policy before AI/workflow/outbound side effects.
- `src/core/trace-store.js`: bounded privacy-minimized operational traces.
- `src/core/`: idempotency, policy, intent, knowledge, AI routing and workflow bridge.
- `src/skills/`: runtime business/customer-service skills and safe dynamic skill registry, not Claude Code instructions.
...
- no secret leaks into logs, traces or persisted Bot Hub state;
- tests cover expected and adversarial behavior;
- skill changes include routing/eval coverage;
- tool-policy changes include deny-path coverage;
- frontend JavaScript passes the syntax gate;
- `npm run check` passes;
- `npm test` passes;
- Docker and packaged Windows gates pass when runtime/deployment files change;
